...
/// Mock implementation of the ChoreServiceProtocol for testing
public class MockChoreService: ChoreServiceProtocol {
    // MARK: - Properties
    ...
    /// The chore validator
    public let choreValidator: ChoreValidatorProtocol
    ...
    /// Creates a new mock chore service
    /// - Parameters:
    ...
    public init(
        ...
        choreGenerator: ChoreGeneratorProtocol = MockChoreGenerator(),
        choreValidator: ChoreValidatorProtocol = MockChoreValidator(),
        choreScheduler: ChoreSchedulerProtocol = MockChoreScheduler(),
        pointAllocationService: PointAllocationServiceProtocol = MockPointAllocationService()
    ) {
    ...
    public func createRecurringChore(
        ...
        // Create the parent chore in the repository
        let parentId = try choreRepository.createChore(
            title: title,
            description: description,
            points: points,
        ...
        guard let parentChore = try choreRepository.getChore(id: parentId) else {
            throw ChoreError.createFailed("Failed to retrieve created parent chore").toAppError()
        }

        // Generate child chores
        let endDate = try choreScheduler.getRecurringChoreEndDate()
        let childChoreModels = try choreGenerator.generateChoreInstances(
            parentChore: parentChore,
            startDate: Date(),
            endDate: endDate
        )

        // Create child chores in the repository
        var childChores: [ChoreModel] = []

        for childModel in childChoreModels {
            let childId = try choreRepository.createChore(
                title: childModel.title,
                description: childModel.description,
                points: childModel.points,
                dueDate: childModel.dueDate,
                isRecurring: false,
                recurringPattern: nil,
                status: childModel.status,
                parentChoreId: parentId,
                assignedToUserId: childModel.assignedToUserId,
                createdByUserId: childModel.createdByUserId,
                familyId: childModel.familyId,
                iconId: childModel.iconId
            )

            if let childChore = try choreRepository.getChore(id: childId) {
                childChores.append(childChore)
            }
        }
    ...
    public func completeChore(
        ...
        let updatedChore = try updateChore(id: id) { chore in
            chore = chore.copy { $0.status = newStatus }
        }
    ...
    public func verifyChore(
        ...
        // Update the chore status
        let updatedChore = try updateChore(id: id) { chore in
            chore = chore.copy { $0.status = .verified }
        }
    ...
    public func rejectChore(
        ...
        // Update the chore status
        let updatedChore = try updateChore(id: id) { chore in
            chore = chore.copy { $0.status = .rejected }
        }
    ...
    public func markChoreMissed(id: Int64) throws -> ChoreModel {
        ...
        // Update the chore status
        let updatedChore = try updateChore(id: id) { chore in
            chore = chore.copy { $0.status = .missed }
        }
