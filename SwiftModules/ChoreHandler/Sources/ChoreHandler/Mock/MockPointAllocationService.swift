...
/// Mock implementation of the PointAllocationServiceProtocol for testing
public class MockPointAllocationService: PointAllocationServiceProtocol {
    // MARK: - Properties
    
    /// In-memory storage for point transactions
    private var pointTransactions: [PointTransaction] = []
    
    /// In-memory storage for user point totals
    private var userPoints: [UUID: Int] = [:]
    
    // MARK: - Initialization
    
    /// Creates a new mock point allocation service
    public init() {}
    ...
    public func allocatePoints(
        ...
        let currentPoints = userPoints[userId] ?? 0
        userPoints[userId] = currentPoints + Int(points)
